...
export default function ProductDetail({ product, onBack, setView }) {
  ...
  const reviews =
    product.reviews || (product.featuredReview ? [product.featuredReview] : []);
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-start mb-24 md:mb-32">
          {/* LEFT: GALLERY – completely unchanged */}
          <div
            ref={leftRef}
            className="lg:col-span-6 space-y-6 lg:sticky lg:top-32 order-1"
          >
            {/* ... gallery thumbnails + main image + zoom + badges ... unchanged ... */}
          ...
          <div className="lg:col-span-6 order-2 mt-8 lg:mt-0">
            <div ref={rightRef} className="space-y-10 lg:overflow-y-auto">
              <header className="space-y-6">
                ...
                <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-ionMidnight tracking-tighter italic uppercase leading-[0.95] md:leading-[0.9]">
                  {product.name}
                </h1>
                ...
                {/* ────────────────────────────────
                     NEW: PRICE + MRP + SHIPPING
                ──────────────────────────────── */}
                <div className="flex flex-wrap items-baseline gap-x-4 gap-y-2">
                  <div className="flex items-baseline gap-3">
                    <span className="text-3xl sm:text-4xl font-black text-ionMidnight">
                      ₹{product.sellingPrice?.toLocaleString("en-IN") || "—"}
                    </span>
                    <span className="text-lg sm:text-xl text-gray-400 line-through decoration-red-400/70">
                      ₹{product.mrp?.toLocaleString("en-IN") || "—"}
                    </span>
                  </div>
                  <span className="text-sm font-medium text-ionGreen bg-ionGreen/10 px-3 py-1 rounded-full">
                    Inclusive • All taxes & shipping
                  </span>
                </div>

                <div className="flex flex-wrap gap-6 text-sm text-ionBlue/70">
                  <div className="flex items-center gap-2">
                    <Truck size={16} className="text-ionGreen" />
                    Delivery: <strong>{product.deliveryTime || "—"}</strong>
                  </div>
                  <div className="flex items-center gap-2">
                    <Clock size={16} className="text-ionGreen" />
                    Ships in: <strong>{product.shippingTime || "—"}</strong>
                  </div>
                </div>
              ...
              {/* SPECS – unchanged + new warranty & weight entries added naturally */}
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                  ...
                  {/* Added warranty & logistics fields – same card style */}
                  <div className="p-4 md:p-5 rounded-2xl md:rounded-3xl bg-white border border-gray-100 hover:shadow-lg transition-all group">
                    <span className="text-[8px] md:text-[9px] font-black text-ionBlue/30 uppercase tracking-widest block mb-1 group-hover:text-ionGreen">
                      Warranty
                    </span>
                    <span className="text-xs md:text-sm font-bold text-ionMidnight">
                      {product.warrantyPeriod} —{" "}
                      {product.warrantyTerms?.[0] || "—"}
                    </span>
                  </div>
            ...
            <div className="relative group">
              ...
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  exit={{ opacity: 0, x: -10 }}
                  className="overflow-hidden rounded-[2rem] md:rounded-[3rem] bg-white border border-gray-100 shadow-xl"
                >
                  <div className="grid grid-cols-1 lg:grid-cols-12 items-stretch">
                    {/* VIDEO SIDE */}
                    {currentReview.video ? (
                      <div
                        className="lg:col-span-4 relative bg-black aspect-video lg:aspect-auto group/video overflow-hidden cursor-pointer"
                      ...
                      >
                        <video
                          ...
                          preload="metadata"
                          poster={
                            currentReview.thumbnail ||
                            `${currentReview.video}#t=0.1`
                          }
                        />
                      ...
                      <div className="max-w-2xl">
                        ...
                        <div className="flex items-center justify-between pt-6 border-t border-gray-100">
                          <div className="flex items-center gap-4">
                            <div className="w-12 h-12 rounded-xl bg-ionBlue text-white flex items-center justify-center font-black text-xl shadow-lg shadow-ionBlue/20">
                              {currentReview.name[0]}
                            </div>
                            <div>
                              <p className="text-sm font-black text-ionBlue uppercase">
                                {currentReview.name}
                              </p>
                              <p className="text-[10px] font-bold text-ionGreen uppercase">
                                {currentReview.location} • {currentReview.role}
                              </p>
                            </div>
                          </div>
                        </div>
        ...
        <AnimatePresence>
          {showVideoPopup && (
            <motion.div
              ...
              className="fixed inset-0 z-[999] bg-ionMidnight/95 backdrop-blur-xl flex items-center justify-center p-4"
            >
              <button
                onClick={(e) => {
                  e.stopPropagation();
                ...
                className="absolute top-24 right-6 md:top-20 w-12 h-12 rounded-full bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center text-white z-[1000]"
              >
                <X size={24} />
              </button>
              <motion.div
